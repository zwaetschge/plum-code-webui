...
await page.goto('http://localhost:4545/session/Y8mOj5_9IU_9qpadX34SM',{waitUntil:'networkidle',timeout:45000}).catch(e=>console.log('goto:',e.message));
await page.waitForTimeout(4000);
console.log(await page.evaluate(() => {
  const f = document.querySelector('.chat-composer-form');
  if(!f) return 'NO FORM';
  const cs = getComputedStyle(f);
  const fadeEl = document.querySelector('.session-global-edge-fade-bottom');
  return JSON.stringify({ bgImage: cs.backgroundImage.slice(0,60), backdrop: cs.backdropFilter, fade: fadeEl ? getComputedStyle(fadeEl).backgroundImage.slice(0,72) : null }, null, 2);
}));
await page.screenshot({ path:'/tmp/plum-shot/live-plain.png', clip:{x:300,y:790,width:1120,height:260} });
await page.evaluate(() => {
...
await page.screenshot({ path:'/tmp/plum-shot/live-proof.png', clip:{x:120,y:820,width:1480,height:230} });
